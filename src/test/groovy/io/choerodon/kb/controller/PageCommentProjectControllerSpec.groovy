...
@ActiveProfiles("test")
@Stepwise
class PageCommentProjectControllerSpec extends Specification {
    ...
    def url = '/v1/projects/{project_id}/page_comment'
    ...
    def setup() {
        if (isFirst) {
            ...
            create.comment = "评论内容"
            createPageCommentId = pageCommentService.create(organizationId, projectId, create).id
        }
    }
    ...
    def "create"() {
        ...
        def entity = restTemplate.exchange(url + "?organizationId=" + organizationId, HttpMethod.POST, httpEntity, PageCommentVO.class, projectId)
    ...
    def "update"() {
        ...
        def entity = restTemplate.exchange(url + "/{id}?organizationId=" + organizationId, HttpMethod.PUT, httpEntity, PageCommentVO.class, projectId, pageComment.id)
    ...
    def "deleteComment"() {
        when:
        '通过id删除评论（管理员权限）'
        def entity = restTemplate.exchange(url + "/{id}?organizationId=" + organizationId, HttpMethod.DELETE, null, ResponseEntity, projectId, createPageCommentId)
    ...
    def "deleteMyComment"() {
        when:
        '通过id删除评论（删除自己的评论）'
        def entity = restTemplate.exchange(url + "/delete_my/{id}?organizationId=" + organizationId, HttpMethod.DELETE, null, ResponseEntity, projectId, pageComment.id)
