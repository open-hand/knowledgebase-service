...
@ActiveProfiles("test")
@Stepwise
class PageVersionOrganizationControllerSpec extends Specification {
    ...
    def url = '/v1/organizations/{organization_id}/page_version'
    ...
    def setup() {
        if (isFirst) {
            ...
            workSpaceInfo = workSpaceService.updateWorkSpaceAndPage(organizationId, null, workSpaceInfo.id, update)
    ...
    def "queryById"() {
        when:
        '查询版本内容'
        def entity = restTemplate.exchange(url + "/{version_id}?pageId=" + workSpaceInfo.pageInfo.id, HttpMethod.GET, null, PageVersionInfoVO.class, organizationId, pageVersions.get(0).id)
    ...
    def "compareVersion"() {
        when:
        '版本比较'
        def entity = restTemplate.exchange(url + "/compare?pageId=" + workSpaceInfo.pageInfo.id + "&&firstVersionId=" + pageVersions.get(0).id + "&&secondVersionId=" + pageVersions.get(0).id, HttpMethod.GET, null, PageVersionCompareVO.class, organizationId)
    ...
    def "rollbackVersion"() {
        when:
        '版本回退'
        def entity = restTemplate.getForEntity(url + "/rollback?pageId=" + workSpaceInfo.pageInfo.id + "&&versionId=" + pageVersions.get(0).id, null, organizationId)
