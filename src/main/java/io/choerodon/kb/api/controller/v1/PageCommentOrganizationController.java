...
public class PageCommentOrganizationController {
    ...
    public ResponseEntity<PageCommentVO> create(@ApiParam(value = "组织ID", required = true)
        ...
        return Results.success(pageCommentService.create(organizationId, null, pageCreateCommentVO));
    }
    ...
    public ResponseEntity<List<PageCommentVO>> queryByList(@ApiParam(value = "组织id", required = true)
        ...
        return Results.success(pageCommentRepository.queryByPageId(organizationId, null, pageId));
    }
    ...
    public ResponseEntity<PageCommentVO> update(@ApiParam(value = "组织ID", required = true)
        ...
        return Results.success(pageCommentService.update(organizationId, null, id, pageUpdateCommentVO));
    }

    @Permission(level = ResourceLevel.ORGANIZATION)
    @ApiOperation("通过id删除评论（管理员权限）")
    @DeleteMapping({"/{id}"})
    public ResponseEntity<Void> deleteComment(@ApiParam(value = "组织ID", required = true)
                                        @PathVariable("organization_id") Long organizationId,
                                        @ApiParam(value = "评论id", required = true)
                                        @PathVariable @Encrypt Long id) {
    ...
    public ResponseEntity<Void> deleteMyComment(@ApiParam(value = "组织ID", required = true)
                                          @PathVariable("organization_id") Long organizationId,
                                          @ApiParam(value = "评论id", required = true)
                                          @PathVariable @Encrypt Long id) {
