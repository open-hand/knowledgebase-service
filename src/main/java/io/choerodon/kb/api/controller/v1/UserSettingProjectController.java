...
public class UserSettingProjectController {
    ...
    @PostMapping
    public ResponseEntity<UserSettingVO> createOrUpdate(@ApiParam(value = "项目id", required = true)
                                         @PathVariable(name = "project_id") Long projectId,
                                         @ApiParam(value = "组织id", required = true)
                                         @RequestParam Long organizationId,
                                         @ApiParam(value = "user setting VO", required = true)
                                         @RequestBody UserSettingVO userSettingVO) {
        return Results.success(userSettingService.createOrUpdate(organizationId, projectId, userSettingVO));
    }
}
