...
To install the chart with the release name `knowledgebase-service`:

```console
$ helm repo add c7n https://openchart.choerodon.com.cn/choerodon/c7n
$ helm repo update
$ helm install knowledgebase-service c7n/knowledgebase-service
```
...
| containerPort.actuatorPort | int | `8281` | server management port |
| containerPort.serverPort | int | `8280` | server port |
...
| extraEnv.ELASTICSEARCH_IP | string | `"elasticsearch-kb:9200"` |  |
| extraEnv.EUREKA_CLIENT_SERVICEURL_DEFAULTZONE | string | `"http://register-server:8000/eureka/"` |  |
| extraEnv.SERVICES_ATTACHMENT_URL | string | `"http://minio.example.com/knowledgebase-service"` |  |
...
| extraEnv.SPRING_DATASOURCE_URL | string | `"jdbc:mysql://localhost:3306/knowledgebase_service?useUnicode=true&characterEncoding=utf-8&useSSL=false&useInformationSchema=true&remarks=true&serverTimezone=Asia/Shanghai"` |  |
| extraEnv.SPRING_DATASOURCE_USERNAME | string | `"choerodon"` |  |
| extraEnv.SPRING_REDIS_DATABASE | int | `14` |  |
...
| image.repository | string | `"c7n/knowledgebase-service"` | service image repository |
...
| service.nodePort | object | `{"actuator":30151,"server":30150}` | Specify the nodePort value for the LoadBalancer and NodePort service types. |
| service.port | object | `{"actuator":8281,"server":8280}` | server Service port |
