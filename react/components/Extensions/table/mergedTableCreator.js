 ...
 * @private
 */
export function extractPropertiesForMerge(value, type, oppossitType) {
 ...
 * @private
 */
export function parseTableCell(cell) {
  const { nodeName } = cell;
 ...
 * @private
 */
export function createTableObjectFrom$Table($table) {
  return $table.find('tr').get().map(tr => $(tr).find('td, th').get().map(parseTableCell));
}
 ...
 * @private
 */
export function divideTrs(trs) {
  const tbodyStartIndex = findIndex(trs, tr => (tr[0].nodeName === 'TD'));
 ...
 * @private
 */
export function mergeByColspan(trs) {
  trs.forEach((tr) => {
 ...
 * @private
 */
export function getRemovalTdCountsByRowspan(trs) {
 ...
 * @private
 */
export function mergeByRowspan(trs) {
  const removalCounts = getRemovalTdCountsByRowspan(trs);
...
export default function createMergedTable(tableElement) {
  const table = createTableObjectFrom$Table($(tableElement));
  const [thead, tbody] = divideTrs(table);

  mergeByColspan(thead);
  mergeByColspan(tbody);
  mergeByRowspan(tbody);
