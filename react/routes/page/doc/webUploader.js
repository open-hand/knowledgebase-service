...
class WebUploader {
  ...
  async init(prefixPatch, organizationId, combineUrl) {
    getCurrentOrganizationId = organizationId;
    this.checkUrl = `${window._env_?.API_HOST}${prefixPatch}/v1/${getCurrentOrganizationId}/upload/check-block`;
    this.uploadUrl = `${window._env_?.API_HOST}${prefixPatch}/v1/${getCurrentOrganizationId}/upload/save`;
    if (combineUrl) {
      this.combineUrl = combineUrl;
    } else {
      this.combineUrl = `${window._env_?.API_HOST}${prefixPatch}/v1/upload/fragment-combine-c7n`;
    }
